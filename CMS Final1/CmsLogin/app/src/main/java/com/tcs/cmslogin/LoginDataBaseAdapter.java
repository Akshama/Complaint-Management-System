...
public ArrayList<String> getCompId()
{
    ...
    /******** METHOD TO GET THE COMPLAINT ID FROM EMP ID ***********/

    public  String getSelect_AssignedCompId(String empID)
    {
        db=dbHelper.getReadableDatabase();
        String complaintID=new String();
        Cursor cursor;
        cursor=db.query("ASSIGN_COMPLAINT", null, " ENGINEER_ID=?", new String[]{empID}, null, null, null);
        if(cursor.getCount()<1) // UserName Not Exist
        {
            cursor.close();
            complaintID ="";
        ...
        cursor.moveToFirst();
        for (int i = 0; i < cursor.getCount(); i++) {
            complaintID=cursor.getString(cursor.getColumnIndex("COMPLAINT_ID"));
            //complaint.add(name);
            cursor.moveToNext();
        }
        cursor.close();
    ...
    public String getSelect_ComplaintStatus1(String id)
    {
        ...
        cursor.moveToFirst();
        for (int i = 0; i < cursor.getCount(); i++) {
            complaint=cursor.getString(cursor.getColumnIndex("COMPLAINT_STATUS"));
            //complaint.add(name);
            cursor.moveToNext();
        }
        cursor.close();
...
//        updatedValues.put("USERNAME", name);
    ...
    public void  updateStatus(String status, String id)
    {
        ...
        ContentValues updatedValues = new ContentValues();
        // Assign values for each row.
        updatedValues.put("COMPLAINT_STATUS",status );

        String where="COMPLAINT_ID = ?";
        db.update("COMPLAINT_TABLE",updatedValues, where, new String[]{id});
    }
